...
/*:
 ...
 - DataController handles reading-data-from and writing-data-to disk (binary format).
...
class SWDataController {
  ...
  class func loadAllCharacters() -> [SWCharacter] {
    ...
    do {
      ...
      let characterFiles = directoryContents.filter {
        $0.hasSuffix(".character")
      }

      let characterNames = characterFiles.flatMap {
        $0.components(separatedBy: ".").first
      }

      let loadedCharacters = characterNames.flatMap {
        load(characterNamed: $0)
      }
      characters = loadedCharacters
    }
    catch {
      return [] // ERROR: just return empty ¯\_(ツ)_/¯
    }
    return characters
  }
...
//: ----
//: # In Action...
//: > Abbreviated JSON dictionary.
let jsonDictionary:[String : AnyObject] = [
  "name": "Luke Skywalker" as AnyObject,
  "eye_color": "blue" as AnyObject,
  "url": "http://swapi.co/api/people/1/" as AnyObject
]

var character = SWDataController.createCharacter(fromJSON: jsonDictionary)

let success = SWDataController.save(character: character!)

character = nil

let allCharacters = SWDataController.loadAllCharacters()

/*: [Model](@previous)  |  [DataSource](@next) */
